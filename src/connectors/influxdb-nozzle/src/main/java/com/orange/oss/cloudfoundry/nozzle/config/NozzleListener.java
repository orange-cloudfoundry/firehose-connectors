...
public class NozzleListener {
	...
	@Value("${firehose.bearer}")
	String firehoseBearer;

	@Value("${firehose.skipTlsValidation}")
	boolean firehoseSkipTlsValidation;

	public NozzleListener() {
		try (final Firehose firehose = FirehoseBuilder.create(this.firehoseUrl, this.firehoseBearer)
